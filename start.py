...
def startDockerService(INSTANCE_URL, INSTANCE_USERNAME, BROWSER, AGENT_ID):
	...
	if BROWSER == 'Edge':
		env.append("BROWSER_OPTIONS=use_chromium=True;headless=True;add_argument('--no-sandbox');add_argument('--disable-gpu')")
	else:
		env.append("BROWSER_OPTIONS=add_argument('--no-sandbox');add_argument('--disable-gpu')")
